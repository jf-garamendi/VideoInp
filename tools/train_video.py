...
parser.add_argument('--out_c', type=int, default=4)
...
def main():
    ...
    for epoch in range(args.n_epochs):
        ...
        for i, (flow_masked,rflow_masked, mask, gt_all_flow, gt_all_rflow, edge, img, bbox) in enumerate(tqdm(train_loader)):
            ...
            gt_flow = gt_all_flow[:,2].cuda()
            gt_rflow=gt_all_rflow[:,3].cuda()
            mask = mask.cuda()
            fmask=mask[:,2]
            bmask=mask[:,3]
            ...
            loss['L1 mask'] = .5*(L1_mask(torch.stack(fflows,dim=4), torch.stack([gt_flow]*len(flows),dim=4),torch.stack([mask[:,2]]*len(flows),dim=4))+L1_mask(torch.stack(bflows,dim=4), torch.stack([gt_rflow]*len(flows),dim=4),torch.stack([mask[:,3]]*len(flows),dim=4)))
   ...
   #                #   l_fake.append(torch.cat((fmask[b,0:1,bbox[0][b]:bbox[0][b]+bbox[2][b],bbox[1][b]:bbox[1][b]+bbox[3][b]],bmask[b,0:1,bbox[0][b]:bbox[0][b]+bbox[2][b],bbox[1][b]:bbox[1][b]+bbox[3][b]],fake[:,bbox[0][b]:bbox[0][b]+bbox[2][b],bbox[1][b]:bbox[1][b]+bbox[3][b]]),dim=0))
                    ...
                    fake_batch_rflow[3]=flows[0][b,2:] * bmask[b,:,:,:] + rflow_masked[b,3,:,:,:] * (1. - bmask[b,:,:,:]) 
            ...
            if i % args.PRINT_EVERY == 0:
                ...
                for j in range(flows[0].shape[0]):#batch
                    for k in range(args.n_frames):#layer
                        if k==2:
                            ffake = (flows[0][j,:2] * mask[j,k,:,:] + flow_masked[j,k,:,:,:] * (1. - mask[j,k,:,:])).detach().cpu().numpy()
                        else:
                            ffake = flow_masked[j,k,:,:,:].detach().cpu().numpy()
                        if k==3:
                            bfake=(flows[0][j,2:] * mask[j,k,:,:] + rflow_masked[j,k,:,:,:] * (1. - mask[j,k,:,:])).detach().cpu().numpy()
                        else:
