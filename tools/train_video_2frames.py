...
def main():
    ...
    if args.net=='interponet':
        ...
        IN=Res(in_c,out_c,7)    
    elif args.net=='warpres':
        IN=WarpRes(in_c,out_c,5,2)
    elif args.net=='unet':
        IN=UNet(in_c,out_c)
    elif args.net=='warpunet':
        IN=Warp_UNet(in_c,out_c)
    ...
    for epoch in range(args.n_epochs):
        if args.GAN or args.patchGAN or args.patchGANmask:
        ...
        for i, (flow_masked,rflow_masked, mask, gt_all_flow, gt_all_rflow, edge, img, bbox) in enumerate(tqdm(train_loader)):
            ...
            flows=IN(input_x)
            fflows=[flows[0][:,:args.out_c//2]]
            bflows=[flows[0][:,args.out_c//2:]]
            ...
            if args.two_masks:
                loss['L1 mask'] = .5*(L1_mask(torch.stack(fflows,dim=4), torch.stack([gt_flow]*len(flows),dim=4),torch.stack([torch.cat((mask[:,2],mask[:,3]),dim=1)]*len(flows),dim=4))+L1_mask(torch.stack(bflows,dim=4), torch.stack([gt_rflow]*len(flows),dim=4),torch.stack([torch.cat((mask[:,2],mask[:,3]),dim=1)]*len(flows),dim=4)))
            else:
                loss['L1 mask'] =.5*(L1_mask(torch.stack([flows[0][:,:out_c//2]],dim=4), torch.stack([gt_all_flow.view(B,N*C,H,W).cuda()]*len(flows),dim=4),torch.stack([mask.view(B,N*C,H,W)]*len(flows),dim=4))+L1_mask(torch.stack([flows[0][:,out_c//2:]],dim=4), torch.stack([gt_all_rflow.view(B,N*C,H,W).cuda()]*len(flows),dim=4),torch.stack([mask.view(B,N*C,H,W)]*len(flows),dim=4)))
   ...
   #                #   l_fake.append(torch.cat((fmask[b,0:1,bbox[0][b]:bbox[0][b]+bbox[2][b],bbox[1][b]:bbox[1][b]+bbox[3][b]],bmask[b,0:1,bbox[0][b]:bbox[0][b]+bbox[2][b],bbox[1][b]:bbox[1][b]+bbox[3][b]],fake[:,bbox[0][b]:bbox[0][b]+bbox[2][b],bbox[1][b]:bbox[1][b]+bbox[3][b]]),dim=0))
                    ...
                    if args.two_masks:
                        ...
                        for t in range(args.n_frames):
                            fake_batch_flow[t]= flows[0][b,2*t:2*t+2]* mask[b,t,:,:] + flow_masked[b,t,:,:,:] * (1. - mask[b,t,:,:])
                            fake_batch_rflow[t]=flows[0][b,out_c//2+2*t:out_c//2+2*t+2] *tmask[b,t,:,:] + rflow_masked[b,t,:,:,:] * (1. - mask[b,t,:,:]) 
                        l_fake.append(torch.cat((fake_batch_flow.view(N*C,H,W),fake_batch_rflow.view(N*C,H,W),mask[b,:,0]),dim=0)[:,bbox[0][b]:bbox[0][b]+bbox[2][b],bbox[1][b]:bbox[1][b]+bbox[3][b]])
            ...
            if i % args.PRINT_EVERY == 0:
                ...
                if not os.path.exists(log_dir+"/fgt"):
                    os.makedirs(log_dir+"/fgt")
                if not os.path.exists(log_dir+"/bgt"):
                    os.makedirs(log_dir+"/bgt")
                ...
                for j in range(flows[0].shape[0]):#batch
                    for k in range(args.n_frames):#layer
                        if args.two_masks:
                            ...
                            ffake = (flows[0][j,2*k:2*k+2] * mask[j,k,:,:] + flow_masked[j,k,:,:,:] * (1. - mask[j,k,:,:])).detach().cpu().numpy()
                            bfake=(flows[0][j,args.out_c//2+k*2:args.out_c//2+2*k+2] * mask[j,k,:,:] + rflow_masked[j,k,:,:,:] * (1. - mask[j,k,:,:])).detach().cpu().numpy()
                        
                        iio.write(log_dir+'/forward/{:02d}_{:02d}.flo'.format(j,k),ffake.transpose(1,2,0))
                        iio.write(log_dir+'/backward/{:02d}_{:02d}.flo'.format(j,k),bfake.transpose(1,2,0))
                        ...
                        iio.write(log_dir+'/output/{:02d}_{:02d}.flo'.format(j,k),flows[0][j,2*k:2*k+2].detach().cpu().numpy().transpose(1,2,0))
