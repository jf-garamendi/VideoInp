...
def gen_flow_initial_list(root,seg_root,boundary_root,output_txt_path,f=""):
        ...
        for i in range(flow_start_no - 5, flow_start_no + flow_num - 5):
            ...
            if True:
                for k in range(11):
                    ...
                    output_txt.write(flow_root+'/%05d.flo' % (flow_no*5)) #5 for VOS
                    output_txt.write(' ')
        
                for k in range(11):
                    flow_no = np.clip(i + k, a_min=flow_start_no, a_max=flow_start_no + flow_num - 1)
                    output_txt.write(root+'/%05d.png' % (flow_no*5))
                    output_txt.write(' ')
                    
                for k in range(11):
                    flow_no = np.clip(i + k, a_min=flow_start_no, a_max=flow_start_no + flow_num - 1)
                    output_txt.write(seg_root+'/%05d.png' % (flow_no*5))
                    output_txt.write(' ')
                
                for k in range(11):
                    flow_no = np.clip(i + k, a_min=flow_start_no, a_max=flow_start_no + flow_num - 1)
                    output_txt.write(boundary_root+'/%05d.png' % (flow_no*5))
                    output_txt.write(' ')
        ...
        for i in range(flow_start_no - 5, flow_start_no + flow_num - 4):
            ...
            if True:
                for k in range(11):
                    ...
                    output_txt.write(flow_root+'/%05d.rflo' % (flow_no*5))
                    output_txt.write(' ')
...
def main():
    ...
    for i,f in enumerate(os.listdir(args.dataset_root)):
        if f in blacklist:
            print(f,"blacklisted")
        elif ('train' in args.output_txt_path) != (i/L<.95):
            ...
            gen_flow_initial_list(frame_dir,seg_root,boundary_root,args.output_txt_path,f)
