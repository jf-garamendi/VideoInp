...
class dataset(torch.utils.data.Dataset):
    def __init__(self, config,val=False,test=False):
        ...
        with open(self.data_list, 'r') as f:
            ...
            for line in tqdm(f):
                flow_dir,img_dir,mask_dir= None,None,None
                line = line.strip()
                line = line.strip(' ')
                ...
                with open(os.path.join(os.path.dirname(flow_dir[0]),'minmax.txt')) as g:
                    for line in g:
                        line=line.rstrip()
                        if line.split(' ')[0]==flow_dir[i]:
                        ...
                        if i==11:
                            break
                i=0
                with open(os.path.join(os.path.dirname(flow_dir[0]),'minmax.txt')) as g: ## looking for rflows, because only flows in datalists, to modify if need include rflows
                    for line in g:
                        
                        line=line.rstrip()
                        if line.split(' ')[0][:-4]==flow_dir[i][:-4]+'r':
                            rm.append(float(line.split(' ')[1]))
                            rM.append(float(line.split(' ')[2]))
                            i+=1
                        if i==11:
                            break
                ...
                if self.config.get_mask:
                    ...
                    if len(m)==11 and len(M)==11 and len(rm)==11 and len(rM)==11:
                        self.data_items.append((flow_dir,img_dir,mask_dir,m,M,rm,rM))
                    # else:
                    #     print(flow_dir[0])
    ...
    def __getitem__(self, idx):
        ...
        for t,i in enumerate(range(-self.n//2+6,6+self.n//2)): 
            ...
            if t==0: 
                if self.config.MASK_MODE == 'bbox':
                    if self.config.bias_edge:
                        ...
                        grad_norm=np.sqrt(np.sum((tmp_flow[1+self.config.MASK_HEIGHT//2:max_h,self.config.MASK_WIDTH//2:max_w-1]-tmp_flow[self.config.MASK_HEIGHT//2:max_h-1,self.config.MASK_WIDTH//2:max_w-1])**2+(tmp_flow[self.config.MASK_HEIGHT//2:max_h-1,1+self.config.MASK_WIDTH//2:max_w]-tmp_flow[self.config.MASK_HEIGHT//2:max_h-1,self.config.MASK_WIDTH//2:max_w-1])**2,axis=2))
                        xx,yy=np.meshgrid(np.arange(grad_norm.shape[0]),np.arange(grad_norm.shape[1]))
                        ind=list(np.stack((xx,yy),axis=-1).reshape(-1,2))

           
                        t,l=ind[sorted(np.arange(len(ind)), key=lambda k:grad_norm[int(ind[k][0]),int(ind[k][1])],reverse=True)[min(len(ind)-1,int(np.random.exponential(0.1*len(ind))))]]
            ...
            if self.config.bbox:
                flow_masked_set.append(tmp_flow_masked[tmp_bbox[0]:tmp_bbox[0]+tmp_bbox[2],tmp_bbox[1]:tmp_bbox[1]+tmp_bbox[3]])
                flow_set.append(tmp_flow[tmp_bbox[0]:tmp_bbox[0]+tmp_bbox[2],tmp_bbox[1]:tmp_bbox[1]+tmp_bbox[3]])
                mask_set.append(np.concatenate((tmp_mask[tmp_bbox[0]:tmp_bbox[0]+tmp_bbox[2],tmp_bbox[1]:tmp_bbox[1]+tmp_bbox[3]],tmp_mask[tmp_bbox[0]:tmp_bbox[0]+tmp_bbox[2],tmp_bbox[1]:tmp_bbox[1]+tmp_bbox[3]]), axis=2))
                
                if self.config.edge is not None:
                    edge_set.append(tmp_boundary[tmp_bbox[0]:tmp_bbox[0]+tmp_bbox[2],tmp_bbox[1]:tmp_bbox[1]+tmp_bbox[3]])
                if self.config.rflow:
                    rflow_masked_set.append(tmp_rflow_masked[tmp_bbox[0]:tmp_bbox[0]+tmp_bbox[2],tmp_bbox[1]:tmp_bbox[1]+tmp_bbox[3]])
                    rflow_set.append(tmp_rflow[tmp_bbox[0]:tmp_bbox[0]+tmp_bbox[2],tmp_bbox[1]:tmp_bbox[1]+tmp_bbox[3]])
                if self.config.image:
                    img_set.append(tmp_img[tmp_bbox[0]:tmp_bbox[0]+tmp_bbox[2],tmp_bbox[1]:tmp_bbox[1]+tmp_bbox[3]])
