...
def bbox2mask(args, bbox):
    ...
    def npmask(bbox, height, width, delta_h, delta_w):
        mask = np.zeros((1, 1, height, width), np.float32)
        h1,h2 = np.random.randint(delta_h // 2 + 1),np.random.randint(delta_h // 2 + 1)
        w1,w2 = np.random.randint(delta_w // 2 + 1),np.random.randint(delta_w // 2 + 1)
        mask[:, :, bbox[0] + h1: bbox[0] + bbox[2] - h2,
        bbox[1] + w1: bbox[1] + bbox[3] - w2] = 1.
        return mask
...
def bbox2mask_perso(args, bbox):
    ...
    def npmask(bbox, height, width, delta_h, delta_w):
        ...
        h1,h2 = np.random.randint((bbox[2]) // 2 - delta_h-5)+delta_h,np.random.randint((bbox[2] ) // 2 - delta_h-5)+delta_h
        w1,w2 = np.random.randint((bbox[3]) // 2 - delta_w-5)+delta_w,np.random.randint((bbox[3]) // 2 - delta_w-5)+delta_w
        mask[:, :, bbox[0] + h1: bbox[0] + bbox[2] - h2,
        bbox[1] + w1: bbox[1] + bbox[3] - w2] = 1.
        return mask
