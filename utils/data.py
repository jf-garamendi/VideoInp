...
class dataset(torch.utils.data.Dataset):
            ...
            for line in f:
                ...
                if self.config.get_mask:
                    ...
                    if self.config.edge is not None:
                        self.data_items.append((flow_dir,boundary_dir, video_class_no))
                    elif self.config.seg is not None:
                        self.data_items.append((flow_dir,seg_dir, video_class_no))
    ...
    def __getitem__(self, idx):
        ...
        if self.config.edge is not None:
            boundary_dir = self.data_items[idx][1]
        elif self.config.seg is not None:
            seg_dir = self.data_items[idx][1]
        video_class_no = self.data_items[idx][2]
        if self.config.get_mask:
            mask_dir = self.data_items[idx][3]
        ...
        if True:
            ...
            if self.config.edge=='grad':
                ...
                img=cv2.resize(img, (self.size[1], self.size[0]),interpolation=cv2.INTER_NEAREST)/ 127.5 - 1
                tmp_seg = img#[:,:,:].reshape(self.size[0],self.size[1],3)
            if self.config.get_mask:
            ...
            if self.config.edge is not None:
                tmp_flow_mask_cat = np.concatenate((tmp_flow_masked, tmp_mask,tmp_boundary), axis=2)
            elif self.config.seg is not None:
                tmp_flow_mask_cat = np.concatenate((tmp_flow_masked, tmp_mask,tmp_seg), axis=2)
        ...
        return flow_mask_cat, flow_masked, gt_flow, mask, tmp_bbox
